...
-- 5. USER CLUSTERING TABLE (for ML-based user grouping)
...
-- 6. CONTENT SIMILARITY TABLE (for article recommendations)
...
-- 7. PERFORMANCE METRICS TABLE (for system monitoring)
...
-- 8. A/B TEST RESULTS TABLE (for optimization testing)
...
-- 9. ARTICLE COMMENTS TABLE (for social features)
...
-- 10. USER FOLLOWS TABLE (for social following system)
...
-- 11. COMMUNITY POSTS TABLE (for user-generated content)
...
-- Insert sample reading goal types for testing
...
FROM (
  VALUES 
    ('daily_reading', 5, 'daily'),
    ('weekly_articles', 25, 'weekly'), 
    ('monthly_minutes', 600, 'monthly'),
    ('category_exploration', 5, 'monthly'),
    ('streak_days', 7, 'continuous')
) AS sample_goals(goal_type, target_value, time_period)
WHERE NOT EXISTS (SELECT 1 FROM reading_goals WHERE goal_type = sample_goals.goal_type);
...
SELECT 
  'user_notifications' AS table_name, COUNT(*) AS row_count FROM user_notifications
UNION ALL
SELECT 'user_sessions', COUNT(*) FROM user_sessions
UNION ALL
SELECT 'user_feedback', COUNT(*) FROM user_feedback
UNION ALL
SELECT 'reading_goals', COUNT(*) FROM reading_goals
UNION ALL
SELECT 'user_clustering', COUNT(*) FROM user_clustering
UNION ALL
SELECT 'content_similarity', COUNT(*) FROM content_similarity
UNION ALL
...
-- SUCCESS MESSAGE
-- ============================================================================
SELECT 'Advanced tables created successfully! Your Bengali News Website now has enhanced user experience, analytics, and social features.' AS status;