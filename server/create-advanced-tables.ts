 ...
 * Creates comprehensive tables for user experience, analytics, and social features
 */

import { supabase } from './supabase';

// Create all advanced tables needed for the application
export async function createAdvancedTables() {
  ...
  const tables = [
    ...
    // Advanced Analytics Tables
    {
      ...
      sql: `
        ...
        DROP POLICY IF EXISTS "Users can view their cluster data" ON user_clustering;
        CREATE POLICY "Users can view their cluster data" ON user_clustering
          FOR SELECT USING (auth.uid() = user_id);
      `
    },
    {
    ...
    },
    {
      ...
      sql: `
        ...
        DROP POLICY IF EXISTS "Users can view their test results" ON ab_test_results;
        CREATE POLICY "Users can view their test results" ON ab_test_results
          FOR SELECT USING (auth.uid() = user_id);
      `
    },
  ...
  for (const table of tables) {
    try {
      ...
      const { error } = await supabase.rpc('exec_sql', { sql_text: table.sql });
...
export async function testAdvancedTables() {
  const tableNames = [
...
// Seed initial data for advanced tables
export async function seedAdvancedTables() {
  console.log('🌱 Seeding advanced tables with initial data...');
  
  // Seed sample reading goals types
  const goalTypes = [
    { goal_type: 'daily_reading', target_value: 5, time_period: 'daily' },
    { goal_type: 'weekly_articles', target_value: 25, time_period: 'weekly' },
    { goal_type: 'monthly_minutes', target_value: 600, time_period: 'monthly' },
    { goal_type: 'category_exploration', target_value: 5, time_period: 'monthly' },
    { goal_type: 'streak_days', target_value: 7, time_period: 'continuous' }
  ];

  // This would be set per user, so we'll create a sample for testing
  // In real implementation, users would create their own goals
  
  return { message: 'Advanced tables ready for user data' };
}