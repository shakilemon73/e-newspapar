...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(`${apiPrefix}/admin/media/:type`, requireAdmin, async (req, res) => {
    try {
      ...
      const filesWithUrls = data?.map(file => ({
        ...file,
        url: supabase.storage.from('media').getPublicUrl(`${type}/${file.name}`).data.publicUrl
      })) || [];
  ...
  app.delete(`${apiPrefix}/admin/media/:id`, requireAdmin, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  // Upload info endpoint for media files
  app.get(`${apiPrefix}/admin/media/upload-info`, requireAdmin, async (req, res) => {
    try {
      return res.json({ 
        message: 'Use client-side upload with Supabase Storage',
        bucketName: 'media',
      ...
      console.error('Error in upload info endpoint:', error);
      return res.status(500).json({ error: error.message });
    }
  });
