/**
 * TEMPORARY ADMIN ROUTES FOR DATABASE SETUP
 * These routes use service role key for direct database access
 * REMOVE THESE ROUTES AFTER SETUP IS COMPLETE
 */

import { Express } from 'express';
import { createClient } from '@supabase/supabase-js';
import { createAdvancedTables, testAdvancedTables, seedAdvancedTables } from './create-advanced-tables';
...
export function setupTemporaryAdminRoutes(app: Express) {
  console.log('⚠️  Setting up temporary admin routes with service role access');
  
  // Create advanced tables
  app.post('/api/temp-admin/create-advanced-tables', async (req, res) => {
    try {
      ...
      const tables = [
        ...
        },
        {
          ...
          sql: `
            CREATE TABLE IF NOT EXISTS user_follows (
              ...
              follow_type VARCHAR(50) DEFAULT 'user',
              is_active BOOLEAN DEFAULT TRUE,
        ...
        },
        {
          ...
          sql: `
            ...
            CREATE INDEX IF NOT EXISTS idx_community_posts_views ON community_posts(views_count);
          `
        }
      ];
      ...
      for (const table of tables) {
        try {
          ...
          const { error } = await supabaseAdmin.rpc('exec_sql', { sql_text: table.sql });
      ...
      res.json({ 
        success: true, 
        message: 'Advanced tables creation completed',
        results,
      ...
      res.status(500).json({ 
        success: false, 
        error: error.message,
        message: 'Failed to create advanced tables'
      });
    }
  });

  // Test advanced tables
  app.get('/api/temp-admin/test-advanced-tables', async (req, res) => {
    try {
      const tableNames = [
        'user_notifications', 'user_sessions', 'user_feedback', 'reading_goals',
        'user_clustering', 'content_similarity', 'performance_metrics', 'ab_test_results',
        'article_comments', 'user_follows', 'community_posts'
      ];
      ...
      for (const tableName of tableNames) {
        try {
          ...
          if (error) {
            ...
            results.push({ table: tableName, exists: true, recordCount: data?.length || 0 });
          }
        } catch (err) {
      ...
      res.json({
        ...
        existingTables,
        totalTables: results.length,
        allTablesExist: existingTables === results.length
      });
  ...
  // Remove temporary admin routes (call this endpoint to clean up)
  app.delete('/api/temp-admin/cleanup', (req, res) => {
    console.log('🧹 Removing temporary admin routes...');
    // This endpoint confirms cleanup - the actual routes are removed by restarting server
    res.json({ 
      success: true, 
      message: 'Temporary admin routes marked for cleanup. Restart server to complete removal.',
      note: 'Remove setupTemporaryAdminRoutes() call from server/routes.ts'
    });
  });

  console.log('✅ Temporary admin routes set up successfully');
  console.log('🔧 Available endpoints:');
  console.log('   POST /api/temp-admin/create-advanced-tables');
  console.log('   GET  /api/temp-admin/test-advanced-tables');
  console.log('   DELETE /api/temp-admin/cleanup');
  console.log('⚠️  Remember to remove these routes after setup!');
}

export function removeTemporaryAdminRoutes() {
  console.log('🧹 Temporary admin routes have been removed for security');
}