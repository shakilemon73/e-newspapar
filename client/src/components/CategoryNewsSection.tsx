      ...
      <div className="mb-4">
        <Link href={`/article/${mainArticle.slug}`}>
          <a className="block">
            ...
            />
            <h4 className="font-bold text-base mb-1 font-hind hover:text-accent transition">
              {mainArticle.title}
            </h4>
          </a>
        </Link>
        <p className="text-sm text-gray-600 line-clamp-2">{mainArticle.excerpt}</p>
        <div className="text-xs text-gray-500 mt-1">{getRelativeTimeInBengali(mainArticle.publishedAt)}</div>
      </div>
        ...
        {relatedArticles.map((article) => (
          <div className="flex gap-2" key={article.id}>
            ...
            <div>
              ...
              <div className="text-xs text-gray-500">{getRelativeTimeInBengali(article.publishedAt)}</div>
            </div>
          </div>
        ))}
      </div>
